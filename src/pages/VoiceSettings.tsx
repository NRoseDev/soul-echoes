...
function browserFallback(text: string, speed: number, volume: number) {
...
export default function VoiceSettingsPage() {
  ...
  const playElevenLabs = useCallback(async (voice: ElevenLabsVoice) => {
    ...
    try {
      ...
      audio.onerror = () => {
        ...
        browserFallback(PREVIEW_TEXT, settings.speed, settings.volume);
      };
      await audio.play();
    } catch (err) {
  ...
  const testVoice = useCallback(() => {
    ...
    else browserFallback(PREVIEW_TEXT, settings.speed, settings.volume);
  }, [settings.elevenLabsVoiceId, settings.speed, settings.volume, playElevenLabs]);
  ...
  return (
    <div className="flex-1 overflow-y-auto p-4 sm:p-6 max-w-2xl mx-auto space-y-8">
      ...
      <section className="space-y-3">
        ...
        <div className="flex gap-3">
          {GENDER_OPTIONS.map((g) => (
            <button key={g.id} onClick={() => setSettings((s) => ({ ...s, genderPref: g.id }))}
              className={`flex-1 py-3 rounded-xl border-2 text-base font-medium transition-all ${
                settings.genderPref === g.id ? "border-primary bg-primary/10 text-foreground" : "border-border bg-card text-muted-foreground hover:border-primary/40"
              }`}>
              {g.label}
            </button>
          ))}
      ...
      <section className="space-y-3">
        <h2 className="font-display text-lg font-semibold text-foreground">Speech Speed — <span className="text-primary">{speedLabel}</span></h2>
        <Slider value={[settings.speed]} onValueChange={([v]) => setSettings((s) => ({ ...s, speed: v }))} min={0.5} max={1.5} step={0.1} />
        <div className="flex justify-between text-xs text-muted-foreground"><span>Slow</span><span>Normal</span><span>Fast</span></div>
      </section>

      <section className="space-y-3">
        <h2 className="font-display text-lg font-semibold text-foreground">Speech Volume — <span className="text-primary">{volumeLabel}</span></h2>
        <Slider value={[settings.volume]} onValueChange={([v]) => setSettings((s) => ({ ...s, volume: v }))} min={0.1} max={1} step={0.05} />
        <div className="flex justify-between text-xs text-muted-foreground"><span>Soft</span><span>Medium</span><span>Loud</span></div>
      </section>

      <section className="space-y-3">
        <h2 className="font-display text-lg font-semibold text-foreground">Available Voices</h2>
          ...
          {filteredVoices.map((voice) => {
            ...
            return (
              <div key={voice.id}
                ...
                onClick={() => setSettings((s) => ({ ...s, elevenLabsVoiceId: voice.id, elevenLabsVoiceName: voice.name }))}
              >
                <button onClick={(e) => { e.stopPropagation(); playElevenLabs(voice); }} disabled={isPlaying}
      ...
      <div className="flex flex-col sm:flex-row gap-3 pb-8">
        <Button onClick={testVoice} variant="outline" size="lg" className="flex-1 rounded-2xl text-base" disabled={playingId !== null}>
          {playingId ? <Loader2 className="h-4 w-4 mr-2 animate-spin" /> : <Play className="h-4 w-4 mr-2" />}
          Test Voice
        </Button>
